...
export class Query {
  ...
  // Question: Can the isEmpty itself go into error?
  // Question: Which condition would isEmpty return 'false'?
  public isEmpty(obj) {
  ...
  public find() {
    ...
    let result
    // Question@line 572: Are you sure that this condition will be for 'content types'?
    if (this.isEmpty(this._query.query) && this._query.query === 'undefined') {
      ...
      return new Promise((resolve, reject) => {
        ...
        let dataPath
        if (this.type === 'asset') {
          dataPath = (!this._query.locale) ? path.join(baseDir, masterLocale, 'assets', '_assets.json') : path.join(
            baseDir, this._query.locale, 'assets', '_assets.json')
        } else {
          dataPath = (!this._query.locale) ? path.join(baseDir, masterLocale, 'data', contentTypeUid,
            'index.json') : path.join(baseDir, this._query.locale, 'data', contentTypeUid, 'index.json')
        }
        ...
        if (!fs.existsSync(dataPath)) {
          ...
          fs.readFile(dataPath, 'utf8', (err, data) => {
            if (err) {
              ...
              let type = "assets"
              // Question@line 612: For assets, we do not need to map 'data'?
              if (this.type !== 'asset') {
